...
export const ExecutorList = () => {
	...
	useEffect(() => {
		...
		const borderAtOverflowY = () => {
			const lastChildY =
				executorList?.lastElementChild?.getBoundingClientRect()
			const firstChildY =
				executorList?.firstElementChild?.getBoundingClientRect()
			const executorListsCoordinates =
				executorList?.getBoundingClientRect()
			if (
				lastChildY &&
				firstChildY &&
				executorList &&
				executorListsCoordinates
			) {
				// -граница иначе дёргается
				executorList.classList.toggle(
					"executor-list_border-bottom",
					lastChildY.bottom - 2 > executorListsCoordinates.bottom,
				)
				executorList.classList.toggle(
					"executor-list_border-top",
					firstChildY.top < executorListsCoordinates.top,
				)
			}
		}
	...
	return (
		<ul
			id="executors-list"
			className="executor-list flex flex-col gap-4 max-h-[400] w-min overflow-y-auto"
		>
