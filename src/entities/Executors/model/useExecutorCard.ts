...
export const useExecutorCard = () => {
	...
	const addSelectedExecutor = useSelectedExecutorsStore(
		(state) => state.addSelectedExecutor,
	)
	const removeSelectedExecutor = useSelectedExecutorsStore(
		(state) => state.removeSelectedExecutor,
	)
	...
	const handleSelect = async (executor: IExecutor) => {
		try {
			const cachedData = cache.get(
				[
					["operations", "executorId"],
					["byExecutor", executor.id],
				].toString(),
			)
			if (!cachedData) {
