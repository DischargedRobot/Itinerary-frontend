...
type RelationKeys = 'itinerary' | 'department' | 'category' | 'type' | 'equipment' | 'executor'
...
export const operationAPI = {
    getOperationByExecutorId: (executorID: number) => {
        const response = APIJSONRequest<IOperationResponse>(`OperationsOfItinerary/by-executor/${executorID}`)
        return response
    }
}