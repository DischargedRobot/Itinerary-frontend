...
interface IOperationFiltersStore extends IOperationFilters {
	...
	setFromDate: (
		dateExecution: NonNullable<
			IOperationFilters["dateExecution"]
		>["fromDate"],
	) => void
	setToDate: (
		dateExecution: NonNullable<
...
export const useOperationFiltersStore = create<IOperationFiltersStore>(
	(set, get) => ({
		productIds: [1],
		isFormed: false,
		dateExecution: {},
		...
		filterArgs: () => ({
			...
			isFormed: get().isFormed,
			dateExecution: get().dateExecution,
		}),
		...
		setFromDate: (fromDate) =>
			set((state) => ({
				dateExecution: { fromDate, toDate: state.dateExecution.toDate },
			})),
		setToDate: (toDate) =>
			set((state) => ({
				dateExecution: {
					fromDate: state.dateExecution.fromDate,
					toDate,
				},
			})),
