 ...
 */
export const PromiseAllNamed = async <T extends Record<string, Promise<unknown>>>(
        promiseObject: T
    ): Promise<{[K in keyof T]: Awaited<T[K]>}> => {
        const entries = Object.entries(promiseObject);

        const results = await Promise.all(entries.map(([key, promise]) => promise.then(result => [key, result])))
